# Este es el script de entrenamiento. Se entrena el agente usando Q-Learning y se
# guarda la tabla Q.
import numpy as np
from config import (
    MAX_MOVS,
...
from game_logic import Agent, Environment, QLearning
...
if __name__ == "__main__":
    ...
    game = Environment(
        n_episodes=NUMBER_OF_EPISODES, max_movements=MAX_MOVS, monster=monster
    )

    # Creamos el algoritmo de entrenamiento
    q_learning = QLearning(
        alpha=Q_ALPHA, gamma=Q_GAMMA, epsilon=Q_EPSILON, epsilon_decay=Q_EPSILON_DECAY
    )
    ...
    for i in range(NUMBER_OF_EPISODES):
        ...
        while not game.is_episode_finished(agent):
            ...
            # Movemos a Doom Guy a la posición inicial, el ambiente toma control del
            # agente momentáneamente
            ...
            if agent_decide:
                ...
                # Le pasamos al algoritmo de Q_learning para que defina la
                # siguiente acción
            ...
            if agent_decide:
                ...
                # Actualizamos la tabla Q del agente, usando el algoritmo de
                # Q-Learning con la recompensa obtenida
