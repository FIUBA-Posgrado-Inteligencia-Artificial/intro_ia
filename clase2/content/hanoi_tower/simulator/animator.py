...
import logic
from constants import ANIM_Y_HIGHEST_POS


class Animator:
    """
    Class responsible for managing animations of disk movement in the Hanoi Tower
    simulation.
    ...
    def __init__(self, hanoi_logic: logic.HanoiBaseLogic, disk_height: int):
        ...
        Parameters:
        - hanoi_logic (logic.HanoiBaseLogic): The logic module for Hanoi Tower
        simulation.
        - disk_height (int): Height of the disk sprite.
        """
    ...
    def get_sequence(self, seq):
        ...
        if seq["type"] == "movement":
            ...
            starting_coordinates = self.hanoi_logic.remove_disk_from_peg(
                seq["peg_start"], self.disk_height
            )
            ending_cordinates = self.hanoi_logic.get_next_disk_position(
                seq["peg_end"], self.disk_height
            )
            ...
            delta_frames_per_part = [
                delta_mov_per_frames(
                    starting_coordinates[1], ANIM_Y_HIGHEST_POS, frames_per_part
                ),
                delta_mov_per_frames(
                    starting_coordinates[0], ending_cordinates[0], frames_per_part
                ),
                delta_mov_per_frames(
                    ANIM_Y_HIGHEST_POS, ending_cordinates[1], frames_per_part
                ),
            ]
        ...
        # Set complete movement information for end sequence
            ...
            self.complete_movement = {"type": seq["type"]}
        ...
        if type_seq == "movement":
            if self.flag_animate:
                ...
                if self.complete_movement["phase"] == 1:
                    # For the first phase, move the disk vertically towards the highest
                    # point
                    delta_y = self.complete_movement["delta_frames_per_part"][0]
                    # Modify dynamically the velocity of movement
                    delta_y, advance_phase = modify_velocity(
                        delta_y, disk_dict[disk_id].center[1], ANIM_Y_HIGHEST_POS
                    )
                    self.complete_movement["delta_frames_per_part"][0] = delta_y

                elif self.complete_movement["phase"] == 2:  # noqa: PLR2004
                    # For the second phase, move the disk horizontally towards the
                    # target peg
                    delta_x = self.complete_movement["delta_frames_per_part"][1]
                    delta_x, advance_phase = modify_velocity(
                        delta_x,
                        disk_dict[disk_id].center[0],
                        self.complete_movement["end_coord"][0],
                    )
                    self.complete_movement["delta_frames_per_part"][1] = delta_x

                else:
                    # For the third phase, move the disk vertically towards the target
                    # peg
                    delta_y = self.complete_movement["delta_frames_per_part"][2]
                    delta_y, advance_phase = modify_velocity(
                        delta_y,
                        disk_dict[disk_id].center[1],
                ...
                if self.complete_movement["phase"] == 4:  # noqa: PLR2004
                    # Clean imperfections
                    disk_dict[disk_id].force_pos_spite(
                        x=self.complete_movement["end_coord"][0],
